...
# Production-ready reminder
def _notify(user, message: str, category="info", email=False):
...
def send_low_mood_alert(user_id: int, entry_id: int):
    ...
    if entry.score <= profile.low_mood_threshold:
        _notify(
            entry.user, f"Your mood today was {entry.score}. Be kind to yourself - try a small, gentle habit.", category="warning", email=True)
...
def schedule_user_habit_reminders(user_id: int):
    ...
    for habit in habits:
        ...
        HabitReminder.objects.update_or_create(user=user, habit=habit, defaults={
                                               "next_trigger_utc": next_utc, "active": True})
...
def process_due_reminders():
    ...
    """
    now = _now_utc()
    due = HabitReminder.objects.select_related(
        "user", "habit", "user__profile").filter(active=True, next_trigger_utc__lte=now)

    for reminder in due:
        # send reminder
        _notify(reminder.user,
                f"Reminder: '{reminder.habit.habit}' - a tiny step today goes a long way.", category="info", email=True)
...
def send_mood_reminder(user_email, mood_level):
    ...
    if mood_level < 3:
        ...
        message = (
            "Keep up the good work! Stay consistent with your habits 🙌"
        )
