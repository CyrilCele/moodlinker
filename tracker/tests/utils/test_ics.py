...
def test_generate_ics_creates_correct_number_of_events(mock_now):
    ...
    assert len(events) == len(
        habits), "ICS should contain one event per active reminder"
...
def test_generate_ics_event_summary_and_description(mock_now):
    ...
    assert event.get(
        "description") == f"Don't forget to complete your habit: {habit.habit}"
...
def test_generate_ics_event_times_match_reminder(mock_now):
    ...
    assert abs(
        dtstart - reminder_time
    ) < delta, "Event start should match next_trigger_utc"
    assert abs(
        dtend - (reminder_time + timedelta(minutes=30))
    ) < delta, "Event end should be 30 mins after start"
